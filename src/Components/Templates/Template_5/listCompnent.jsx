...
const ListViewComponent = (header_para, details_para, tag_id, section_id) => {
  ...
  if (!document.getElementById(section_id + "_listItemElement_" + tag_id)) {
    ...
    listItemElement.setAttribute(
      "class",
      "pt-2 text-[14px] font-bold text-gray-500 capitalize"
    );
    listItemElement.setAttribute(
      "id",
...
const ChangePosition = (align, section_id) => {
  ...
  if (align == "left") {
    ...
    const _ListHeaderElement = document.getElementById(`${section_id}_ListHeaderElement`)
    _ListHeaderElement.classList.replace('text-indigo-950','text-white')
    resumeForm_L.appendChild(parentDiv);
  } else {
    const _ListHeaderElement = document.getElementById(`${section_id}_ListHeaderElement`)
    _ListHeaderElement.classList.replace('text-white','text-indigo-950')
